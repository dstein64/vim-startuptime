...
function! s:Profile(onfinish, onprogress, options, tries, file, items) abort
  ...
  if a:options.input_file isnot# v:null || a:tries <# a:options.tries
    while 1
      try
  ...
  if a:tries ==# 0 || a:options.input_file isnot# v:null
    if has('nvim-0.9')
    ...
    if a:options.input_file is# v:null && a:options.tries !=# len(a:items)
...
function! s:ShowZeroProgressMsg(winid, bufnr, options)
  ...
  if a:options.input_file isnot# v:null | return | endif
...
function! s:Options(args) abort
  ...
  while l:idx <# len(a:args)
    ...
    let l:idx += 1
  endwhile
  if l:options.tries isnot# v:null && l:options.input_file isnot# v:null
  ...
  if l:options.tries is# v:null && l:options.input_file is# v:null
    let l:options.tries = g:startuptime_tries
  endif
  if l:options.tries isnot# v:null
...
function! s:ValidateInputFile(file) abort
  ...
  for l:line in l:input
    if !l:valid_format && stridx(l:line, 'times in msec') !=# -1
      let l:valid_format = v:true
    endif
    let l:editor_text = '--- VIM STARTING ---'
    if has('nvim')
      let l:editor_text = '--- NVIM STARTING ---'
    endif
    if !l:valid_editor && stridx(l:line, l:editor_text) !=# -1
      let l:valid_editor = v:true
    endif
  endfor
  if !l:valid_format
  ...
  if !l:valid_editor
    " Neovim startuptime files require additional handling (Neovim #23036,
    " #26790). We don't support analzying startuptime files that weren't
    " generated by the same editor in use.
    throw 'vim-startuptime: unsupported file format'
  endif
  return v:true
endfunction
...
function! startuptime#StartupTime(mods, ...) abort
  ...
  let l:items = []
  let l:file = tempname()
  if l:options.input_file isnot# v:null
